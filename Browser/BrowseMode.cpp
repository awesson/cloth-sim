...
static const double DT = .5;
// cloth will be an NxN sized cloth
static const int N = 20;
static const double radius = 1.0;
static const double V = .03;

std::vector<Particle*> pVector;

BrowseMode::BrowseMode() {
	camera = make_vector(10.0f, 10.0f, 10.0f);
	target = make_vector(2.5f, 7.5f, 20.0f);
	obstacle_pos = make_vector(2.5, 7.0, 13.0);
	obstacle_vel = make_vector(0.0, 0.0, 0.0);
	track = false;
	current_pose.clear();
	current_motion = 0;
	time = 0.0f;
	...
	cloth = new System(N);
	integrator = new RK4Integrator(*cloth, DT);
	
	// allocate space for the particles stored here so that get_state does not write to uninitialized memory
	pVector.resize(cloth->size());
	for(int i = 0; i < cloth->size(); ++i){
		pVector[i] = new Particle(make_vector(0.0, 0.0, 0.0), 1.0, 1);
	}
	
	dump_frames = false;
	frame_number = 0;
	
	buffer = (unsigned char *) malloc(Graphics::screen_x * Graphics::screen_y * 4 * sizeof(unsigned char));
}
...
void BrowseMode::update(float const elapsed_time) {
	// move forward by DT
	cloth->get_state(pVector);
	integrator->integrate(*cloth, DT);
	
	int count = 0;
	// collision resolution. Use a radius slightly larger than the actual sphere since the cloth
	// has a thickness and the squares in-between particles on the cloth can be intersected otherwise
	while(cloth->resolve_collisions(pVector, obstacle_pos, obstacle_vel, radius + 3*del_x) && count < 10){
		count++;
		// pVector now has the updated velocity so reset and try moving forward by DT again
		cloth->set_state(pVector);
		integrator->integrate(*cloth, DT);
	}
...
void BrowseMode::handle_event(SDL_Event const &event) {
	...
	if (event.key.keysym.sym == SDLK_j) {
		if(event.type == SDL_KEYDOWN) {
			Vector3f vec = target - camera;
			Vector3f right = cross_product(vec, make_vector<float>(0.0f,1.0f,0.f));
			Vector3f vel = right * V / length(right);
			obstacle_vel -= make_vector((double) vel[0],(double) vel[1],(double) vel[2]);
		} else if(event.type == SDL_KEYUP) {
			obstacle_vel = make_vector(0.0f, 0.0f, 0.0f);
		}
	}
	...
	if (event.key.keysym.sym == SDLK_k) {
		if(event.type == SDL_KEYDOWN) {
			Vector3f vec = target - camera;
			Vector3f right = cross_product(vec, make_vector<float>(0.0f,1.0f,0.f));
			Vector3f down = cross_product(vec, right);
			Vector3f vel = down * V / length(down);
			obstacle_vel += make_vector((double) vel[0],(double) vel[1],(double) vel[2]);
		} else if(event.type == SDL_KEYUP) {
			obstacle_vel = make_vector(0.0f, 0.0f, 0.0f);
		}
...
void BrowseMode::draw() {
	...
	if (dump_frames) {
		...
		// 	if ((frame_number % FRAME_INTERVAL) == 0) {
			...
			//unsigned char * buffer = (unsigned char *) malloc(Graphics::screen_x * Graphics::screen_y * 4 * sizeof(unsigned char));
			// if (!buffer)
			// 			exit(-1);
			// glRasterPos2i(0, 0);
			glReadPixels(0, 0, Graphics::screen_x, Graphics::screen_y, GL_RGBA, GL_UNSIGNED_BYTE, buffer);
			for(int i = 0; i < Graphics::screen_y; ++i){
				for(int k = 0; k < Graphics::screen_x; ++k){
					// set alpha value to 255 since it is by default at 0 which would output no color
					buffer[4*i*Graphics::screen_x + 4*k + 3] = 255;
				}
			}
			char filename[13];
			sprintf(filename, "img%.5i.png", frame_number / FRAME_INTERVAL);
			printf("Dumped %s.\n", filename);
			saveImageRGBA(filename, buffer, Graphics::screen_x, Graphics::screen_y);
		// }
		frame_number++;
	}
